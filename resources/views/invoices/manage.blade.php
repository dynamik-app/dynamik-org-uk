                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                ...
                                <div class="bg-white border border-gray-200 rounded-lg p-4 h-full flex flex-col justify-between">
                                    <div>
                                        ...
                                        <div class="text-3xl font-bold text-gray-900" id="invoice-total">£0.00</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!items.length) {
            items.push({ item_type: 'custom', name: '', description: '', quantity: 1, unit_price: 0, catalog_item_id: null });
        }
                ...
                wrapper.innerHTML = `
                    ...
                    <div class="grid grid-cols-1 gap-2 mt-3">
                        ...
                        <div class="flex items-center justify-between text-sm text-gray-600">
                            <span data-line-total>Line total: £${lineTotal(item).toFixed(2)}</span>
        ...
        function updateTotals() {
            ...
            itemsContainer.querySelectorAll('[data-item-row]').forEach((row, index) => {
                ...
                items[index] = { item_type: type, catalog_item_id: catalogId, name, description, quantity, unit_price: price };

                const rowTotal = quantity * price;
                total += rowTotal;
                const totalLabel = row.querySelector('[data-line-total]');
                if (totalLabel) {
                    totalLabel.textContent = `Line total: £${rowTotal.toFixed(2)}`;
                }
            });
