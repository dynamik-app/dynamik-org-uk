        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-8 border-b border-gray-100">
                    <h3 class="text-2xl font-semibold text-gray-900">{{ __('New permission') }}</h3>
                    <p class="mt-1 text-gray-600">{{ __('Define a permission and optionally assign it to existing roles.') }}</p>
                </div>

                <form action="{{ route('admin.permissions.store') }}" method="POST" class="p-8 space-y-6">
                    @csrf
                        ...
                        <div class="mt-3 grid gap-3 sm:grid-cols-2">
                            @forelse ($roles as $role)
                                <label class="flex items-start gap-3 rounded-lg border border-gray-200 p-4 hover:border-indigo-400">
                                    <input type="checkbox" name="roles[]" value="{{ $role->name }}" class="mt-1 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ in_array($role->name, old('roles', [])) ? 'checked' : '' }}>
